...
interface DeleteEmployeeDrawerProps {
...
export default function DeleteEmployeeDrawer({ employee, loading, onClose, onConfirm }: DeleteEmployeeDrawerProps) {
  return (
    <Drawer open={!!employee} onClose={() => !loading && onClose()} side="right" size={400}>
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-3 px-5 py-4 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)]">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-base font-semibold leading-tight">Delete Employee</div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <p className="text-sm text-muted-foreground">Are you sure you want to delete this employee?</p>
          <div className="mt-4 flex flex-col gap-3 rounded-lg border border-foreground/10 bg-muted/30 p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Name</span>
              <span className="text-sm font-medium">{employee?.name ?? "N/A"}</span>
            </div>
            <div className="flex items-center justify-between">
