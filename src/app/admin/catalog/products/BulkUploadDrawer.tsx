...
import { validateBulkUploadProducts, uploadBulkProducts } from "@/services/bulkUpload/products";
...
export default function BulkUploadDrawer({ open, onClose, onUploaded }: BulkUploadDrawerProps) {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const res = await validateBulkUploadProducts(formData);
  ...
  const handleUpload = async () => {
    ...
    try {
      const formData = new FormData();
      formData.append("file", file);
      const res = await uploadBulkProducts(formData);
    ...
    <Drawer open={open} onClose={step === "uploading" ? undefined : onClose} side="right" size={480}>
      <div className="flex h-full flex-col">
        <div className="px-5 py-4 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)]">
          <div className="text-base font-semibold leading-tight">Bulk Product Upload</div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <label
            htmlFor="bulk-upload-products-file"
            ...
            <>
              ...
              {canUpload ? (
                <Button onClick={handleUpload} disabled={(step as Step) === "uploading"}>
                  ...
                  Upload {validationResult?.validRowsCount ?? ""} Products
                </Button>
              ) : (
