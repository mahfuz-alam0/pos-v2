...
import PackageDetailsPanel from "../../packages/PackageDetailsPanel";
import type { PackageRow, StorageLocationOption } from "../../packages/types";
...
export default function MetrcPackagesPage() {
  ...
  const openRow = (id: string) => {
    ...
    params.set("id", id);
    router.push(`/admin/inventory/metrc-reconciliation/packages?${params.toString()}`, { scroll: false });
  };

  const closeDetail = () => {
    const params = new URLSearchParams(searchParams.toString());
    params.delete("id");
    router.push(`/admin/inventory/metrc-reconciliation/packages${params.toString() ? `?${params}` : ""}`, { scroll: false });
  };
