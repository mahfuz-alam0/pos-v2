...
import TransferSelectedPackagesDrawer, {
  type TransferPackageRow,
} from "../transfers/TransferSelectedPackagesDrawer";
...
function buildQueryParams(
  ...
  if (filters.productCategoryIds)
    params.productCategoryIds = filters.productCategoryIds;
  if (filters.productBrandIds) params.productBrandIds = filters.productBrandIds;
  if (filters.storageLocationId)
    params.storageLocationId = filters.storageLocationId;
  ...
  if (filters.lastUpdatedWithinDays)
    params.lastUpdatedWithinDays = filters.lastUpdatedWithinDays;
  if (filters.lastManuallyAdjustedWithinDays)
    params.lastManuallyAdjustedWithinDays =
...
export default function PackagesPage() {
  ...
  const isAdmin =
    userInfo?.type === "SUPER_ADMIN" || userInfo?.type === "ADMINISTRATION";
  ...
  const [transferPackages, setTransferPackages] = useState<
    TransferPackageRow[] | null
  >(null);

  const [reconcileDetail, setReconcileDetail] = useState<any>(null);
  const [reconcileLoadingId, setReconcileLoadingId] = useState<string | null>(
    null,
  );
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      setCategories(
        (catRes?.data ?? []).map((c: any) => ({ id: c.id, name: c.name })),
      );
      setBrands(
        (brandRes?.data ?? []).map((b: any) => ({ id: b.id, name: b.name })),
      );
  ...
  const loadPackages = useCallback(
    async (page = 1, limit = pagination.pageSize) => {
      ...
      try {
        if (tab === "archived") {
          const res = await fetchArchivedPackages(shopId as string, {
            page,
            limit,
          });
          ...
          const res = await fetchPackagesMinimalExtended(
            shopId as string,
            params,
          );
  ...
  const loadCleanupRecord = useCallback(() => {
    ...
    fetchInventoryCleanupRecord(shopId as string)
      .then((res) =>
        setCleanupPackageIds(res?.data?.data?.stagedPackageIds ?? []),
      )
    ...
    async (value: string) => {
      ...
      try {
        ...
        const pkg =
          res?.data?.packages?.[0] ??
          (
            await fetchPackagesMinimalExtended(shopId as string, {
              page: 1,
              limit: 50,
              metrcTags: trimmed,
              isFinished: false,
  ...
  const isRowSelected = (rowId: string | number) =>
    selectedRowKeys.includes(rowId);

  const toggleRow = (row: PackageRow, checked: boolean) => {
    setSelectedRowKeys((prev) =>
      checked ? [...prev, row.id] : prev.filter((k) => k !== row.id),
    );
  ...
  const isCaliforniaState =
    typeof window !== "undefined" &&
    localStorage.getItem("isCaliforniaState") === "true";
  const selectedMetrcRows = selectedRows.filter(
    (r) => r.source === "METRC" && (r.quantityLeft ?? 0) > 0,
  );
  const selectedTransferRows = selectedRows.filter(
    (r) =>
      r.storageLocationBreakdown &&
      Object.keys(r.storageLocationBreakdown).length > 0,
  );

  const openRow = (id: string) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("id", id);
    router.push(`/inventory-management/packages?${params.toString()}`, {
      scroll: false,
    });
  };
  ...
  const closeDetail = () => {
    ...
    params.delete("id");
    router.push(
      `/inventory-management/packages${params.toString() ? `?${params}` : ""}`,
      { scroll: false },
    );
  };
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (filters.searchText)
      chips.push({ key: "searchText", label: `Search: ${filters.searchText}` });
    if (filters.productCategoryIds) {
      const c = categories.find((c) => c.id === filters.productCategoryIds);
      chips.push({
        key: "productCategoryIds",
        label: `Category: ${c?.name ?? filters.productCategoryIds}`,
      });
    }
    if (filters.productBrandIds) {
      ...
      chips.push({
        key: "productBrandIds",
        label: `Brand: ${b?.name ?? filters.productBrandIds}`,
      });
    }
    if (filters.storageLocationId) {
      chips.push({
        key: "storageLocationId",
        label: `Location: ${locationMap[filters.storageLocationId] ?? filters.storageLocationId}`,
      });
    }
    if (filters.discrepancyFilter)
      chips.push({
        key: "discrepancyFilter",
        label: `METRC Discrepancy: ${filters.discrepancyFilter}`,
      });
    if (filters.source)
      chips.push({
        key: "source",
        label: `Source: ${filters.source === "PLATFORM" ? "POS" : "METRC"}`,
      });
    if (filters.packageStatus) {
      const statusLabel =
        filters.packageStatus === "pendingImport"
          ? "Pending Import"
          : filters.packageStatus;
      chips.push({ key: "packageStatus", label: `Status: ${statusLabel}` });
    }
    if (filters.productProfile)
      chips.push({
        key: "productProfile",
        label: `Type: ${filters.productProfile}`,
      });
    if (filters.lastUpdatedWithinDays)
      chips.push({
        key: "lastUpdatedWithinDays",
        label: `Active within ${filters.lastUpdatedWithinDays}d`,
      });
    if (filters.lastManuallyAdjustedWithinDays)
      chips.push({
        key: "lastManuallyAdjustedWithinDays",
        label: `Adjusted within ${filters.lastManuallyAdjustedWithinDays}d`,
      });
    return chips;
  ...
  const handleExport = async (type: "csv" | "xls") => {
    ...
    try {
      ...
      toast.success(
        `${type.toUpperCase()} exported successfully (${allData.length} records)`,
      );
    } catch {
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex min-w-0 flex-nowrap items-center gap-2 overflow-x-auto">
          {selectedRowKeys.length > 0 && (
            <Badge
              variant="outline"
              className="gap-1.5 border-blue-200 bg-blue-50 py-1 pr-1 pl-2.5 font-normal text-blue-700 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-400">
          ...
          {selectedRowKeys.length > 0 && tab === "finishPackages" && (
            <Button
              className="h-9! rounded! px-3.5! text-[14px]! font-medium!"
              variant="outline"
              onClick={() => setBulkFinishOpen(true)}>
          ...
          {cleanupPackageIds.length > 0 && (
            <button
              ...
              <Badge
                variant="outline"
                className="border-amber-300 bg-amber-50 text-amber-700 hover:bg-amber-100 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-400">
          ...
          {shouldPopulateMetrcData ? (
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                  <Button className="h-9! rounded! px-3.5! text-[14px]! font-medium!">
                    Add
                  </Button>
                }
              />
              <DropdownMenuContent>
                <DropdownMenuItem
                  render={<Link href="/inventory-management/packages/add" />}>
                  Create Regular Package
                </DropdownMenuItem>
                <DropdownMenuItem
                  render={
                    <Link href="/inventory-management/packages/import-metrc" />
                  }>
                  Import From METRC
                </DropdownMenuItem>
              </DropdownMenuContent>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex h-10 items-center overflow-hidden rounded-lg border border-input">
              ...
              <Select
                items={[
                  ...
                  { value: "packageName", label: "Product Name" },
                ]}
                value={filters.searchType}
                onValueChange={(v) =>
                  setFilters((prev) => ({
                    ...prev,
                    searchType: v as PackageFilters["searchType"],
                  }))
                }>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="packageName">Product Name</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <ApiSelect
              ...
              onChange={(val) =>
                setFilters((prev) => ({
                  ...prev,
                  productCategoryIds: (val as string) ?? undefined,
                }))
              }
              fetchPage={async (page, search) => {
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...prev,
                  storageLocationId:
                    v === "__all__" ? undefined : (v as string),
                }))
              }>
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...prev,
                  discrepancyFilter:
                    v === "__all__" ? undefined : (v as "YES" | "NO"),
                }))
              }>
              <SelectTrigger className="h-10! w-46">
                <SelectValue
                  className="text-muted-foreground/60"
                  placeholder="Metrc Discrepancy">
                  {(value: string) =>
                    value === "__all__"
                      ? "Metrc Discrepancy"
                      : value === "YES"
                        ? "Has METRC Discrepancy"
                        : "No METRC Discrepancy"
                  }
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...prev,
                  source:
                    v === "__all__" ? undefined : (v as "PLATFORM" | "METRC"),
                }))
              }>
              ...
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...
                  packageStatus:
                    v === "__all__"
                      ? undefined
                      : (v as PackageFilters["packageStatus"]),
                }))
              }>
            ...
            <Select
              ...
              onValueChange={(v) =>
                setFilters((prev) => ({
                  ...prev,
                  productProfile:
                    v === "__all__" ? undefined : (v as "REGULAR" | "CANNABIS"),
                }))
              }>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  ...
                  if (!checked)
                    setFilters((prev) => ({
                      ...prev,
                      lastUpdatedWithinDays: undefined,
                    }));
                }}
              />
            ...
            {showLastUpdated && (
              <Select
                ...
                value={
                  filters.lastUpdatedWithinDays
                    ? String(filters.lastUpdatedWithinDays)
                    : undefined
                }
                onValueChange={(v) =>
                  setFilters((prev) => ({
                    ...prev,
                    lastUpdatedWithinDays: Number(v),
                  }))
                }>
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  ...
                  if (!checked)
                    setFilters((prev) => ({
                      ...prev,
                      lastManuallyAdjustedWithinDays: undefined,
                    }));
                }}
              />
              Last adjusted
            ...
            {showLastAdjusted && (
              <Select
                ...
                value={
                  filters.lastManuallyAdjustedWithinDays
                    ? String(filters.lastManuallyAdjustedWithinDays)
                    : undefined
                }
                onValueChange={(v) =>
                  setFilters((prev) => ({
                    ...prev,
                    lastManuallyAdjustedWithinDays: Number(v),
                  }))
                }>
                <SelectTrigger className="h-9! w-24">
                  <SelectValue placeholder="Days" />
                ...
                <>
                  <TableHead className="w-10">
                    <Checkbox
                      checked={
                        activeRows.length > 0 &&
                        selectedRowKeys.length === activeRows.length
                      }
            ...
            {loading &&
              ...
              Array.from({ length: 8 }).map((_, i) => (
                <TableRow key={`sk-${i}`} className="border-b-0">
                  {Array.from({
                    length:
                      tab === "archived" ? 3 : showMetrcQtyColumn ? 11 : 10,
                  }).map((__, j) => (
                    <TableCell key={j}>
            ...
            {tab === "archived"
              ...
              : activeRows.map((row, i) => (
                  <TableRow
                    ...
                    <TableCell
                      className="max-w-70 whitespace-normal"
                      title={row.name}>
                      {row.name || "-"}
                    </TableCell>
                    <TableCell
                      className={
                        row.metrcTag
                          ? "max-w-40 cursor-pointer truncate transition-colors hover:text-primary"
                          : ""
                      }
                      title={
                        row.metrcTag ? `${row.metrcTag} (Click to copy)` : ""
                      }
                      onClick={() => {
                        if (row.metrcTag)
                          navigator.clipboard.writeText(row.metrcTag);
                      }}>
                      {row.metrcTag || "-"}
                    </TableCell>
      ...
      <PackageDetailsPanel
        ...
        onEditPricing={(inventoryId) =>
          router.push(
            `/inventory-management/inventory-and-pricing/edit/${inventoryId}`,
          )
        }
