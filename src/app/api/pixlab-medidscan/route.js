// Proxy to PixLab docscan for medical ID cards — ported from old POS pages/api.
...
export async function POST(request) {
  try {
    const { img, pixlabApiKey } = await request.json();
    ...
    const pixlabRes = await fetch("https://api.pixlab.io/docscan", {
      ...
      body: JSON.stringify({ img, type: "idcard", country: "usa", key }),
    });
    ...
    if (responseData && responseData.status === 200) {
      ...
      if (responseData.fields) {
        ...
        processedData.doc = {
          ...
          medical_license: fields.documentNumber || "",
          first_name,
          last_name,
        ...
        if (fields.address) {
          const addressParts = fields.address.split(",");
          if (addressParts.length >= 2) {
            const stateZipPart = addressParts[addressParts.length - 1].trim();
            const stateZipMatch = stateZipPart.match(/([A-Z]{2})\s+(\d{5}(-\d{4})?)/);
            if (stateZipMatch) {
              processedData.doc.state = stateZipMatch[1];
              processedData.doc.postal_code = stateZipMatch[2];
            }
            processedData.doc.city = addressParts[addressParts.length - 2].trim();
          }
        }

        // Patient ID pattern specific to medical marijuana cards
        if (!processedData.doc.medical_license) {
          const patientIdMatch =
            fields.fullName?.match(/AP-[A-Z0-9-]+/i) ||
            fields.documentNumber?.match(/AP-[A-Z0-9-]+/i) ||
            fields.address?.match(/AP-[A-Z0-9-]+/i);
          if (patientIdMatch) processedData.doc.medical_license = patientIdMatch[0];
        }
      }
