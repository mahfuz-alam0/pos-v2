...
function TabletPosInner() {
  ...
  const posResetKey = useSelector((state) => state?.lineItems?.resetKey || 0);
  ...
  const customerInQueue = useSelector(
    (state) => state?.customerQueue?.customerInQueue
  );
    ...
    <TooltipProvider>
      <div className={wrapperClass}>
        ...
        <PosDlPhotoManager
          ...
          onScanDL={() => toast.info("DL barcode scanning is pending migration")}
