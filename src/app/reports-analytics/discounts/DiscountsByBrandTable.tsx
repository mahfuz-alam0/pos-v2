...
import type { BrandDiscountRow, DiscountsPagination } from "./types";
...
type SortKey = keyof BrandDiscountRow;
...
export default function DiscountsByBrandTable({
  data,
  loading,
  pagination,
  onPageChange,
  ...
  return (
    <Card className="h-full p-0 shadow-sm ring-0">
      <div className="flex items-center gap-3 px-6 py-4 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)] dark:shadow-[inset_0_-1px_0_rgba(255,255,255,0.08)]">
        ...
        <h3 className="text-base font-semibold">Item Discounts by Brand</h3>
      </div>
        ...
        <Table>
          <TableHeader className="[&_tr]:border-b-0">
            <TableRow className="bg-muted/60">
              <SortHead label="Brand" sortField="brandName" />
              ...
              <SortHead label="Total Discount Amount" sortField="totalItemDiscountAmount" className="text-center" />
          ...
          <TableBody>
            ...
            {sorted.map((row, i) => (
              <TableRow key={row.brandName} className={`border-b-0 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)] ${i % 2 === 1 ? "bg-stone-50 dark:bg-stone-900/40" : ""}`}>
                <TableCell className="font-medium">{row.brandName}</TableCell>
                <TableCell className="text-center">{fmt(row.netSales)}</TableCell>
                ...
                <TableCell className="text-center">{fmt(row.totalItemDiscountAmount)}</TableCell>
                <TableCell className="p-0">
