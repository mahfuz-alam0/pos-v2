...
import { fetchSalesByLocation } from "@/services/reporting/salesByLocation";
...
import {
  ...
  buildLocationHtml,
  buildLocationExcelSheets,
  exportLocationToCsv,
} from "./exportConfig";
import type { SalesByLocationRow, ReportPagination } from "./types";
...
export default function SalesByLocationReport() {
  ...
  const [exportRows, setExportRows] = useState<SalesByLocationRow[]>([]);
  ...
  const fetchDetail = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        toast.error(err?.message || "Failed to load sales by location");
      } finally {
        setLoading(false);
      }
    },
    ...
    try {
      ...
      for (let i = 0; i < totalBatches; i += 5) {
        ...
        const results = await Promise.all(
          pages.map((page) => fetchSalesByLocation(buildFilters({ page, limit: EXPORT_BATCH_SIZE }))),
        );
        results.forEach((res) => {
  ...
  const handleExportCsv = async () => {
    ...
    exportLocationToCsv(all, dateRangeLabel, `sales_by_location_${todayStr()}.csv`);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {runReport && (
        <SalesByTable
          ...
          columns={[
            ...
            { key: "transactionAvg", label: "Transaction Avg", align: "right", render: (r) => money(r.transactionAvg) },
            {
              key: "avgItemPerTransaction",
              label: "Avg Items/Transaction",
              align: "right",
              render: (r) => (r.avgItemPerTransaction ?? 0).toFixed(2),
            },
            {
              key: "loyaltyRedemptions",
              label: "Loyalty Redemptions",
              align: "right",
              render: (r) => String(r.loyaltyRedemptions ?? 0),
              total: (rows) => rows.reduce((s, r) => s + (r.loyaltyRedemptions || 0), 0),
            },
            {
              key: "cash",
              label: "Cash",
              align: "right",
              render: (r) => money(r.cash),
              total: (rows) => money(rows.reduce((s, r) => s + (r.cash || 0), 0)),
            },
            {
              key: "debit",
              label: "Debit",
              ...
              total: (rows) => money(rows.reduce((s, r) => s + (r.debit || 0), 0)),
            },
          ]}
        />
      )}
      ...
      <PdfExportDrawer
        ...
        availableSections={LOCATION_SECTIONS}
        htmlGenerator={buildLocationHtml as any}
        columnConfig={LOCATION_COLUMN_CONFIG}
      />
      <ExcelExportDrawer
        open={excelOpen}
        ...
        metadata={exportMetadata}
        availableSections={LOCATION_SECTIONS}
        excelGenerator={buildLocationExcelSheets as any}
        columnConfig={LOCATION_COLUMN_CONFIG}
        filename={`Sales_By_Location_${todayStr()}`}
      />
    </div>
  );
}
