...
import type { DayOfWeekRow, DayAndTimePagination } from "./types";

function fmt(v: number) {
  return `$${(Number(v) || 0).toFixed(2)}`;
}

type SortKey = keyof DayOfWeekRow;
...
export default function DayOfWeekTable({
  data,
  loading,
  pagination,
  onPageChange,
  ...
  return (
    <Card className="h-full p-0 shadow-sm ring-0">
      <div className="flex items-center gap-3 px-6 py-4 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)] dark:shadow-[inset_0_-1px_0_rgba(255,255,255,0.08)]">
        ...
        <h3 className="text-base font-semibold">Day of Week</h3>
      </div>
        ...
        <Table>
          <TableHeader className="[&_tr]:border-b-0">
            <TableRow className="bg-muted/60">
              <SortHead label="Day" sortField="day" />
          ...
          <TableBody>
            ...
            {sorted.map((row, i) => (
              <TableRow key={row.day} className={`border-b-0 shadow-[inset_0_-1px_0_rgba(0,0,0,0.06)] ${i % 2 === 1 ? "bg-stone-50 dark:bg-stone-900/40" : ""}`}>
