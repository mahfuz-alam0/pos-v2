...
const DELIVERY_ICON = { DELIVERY: Truck, PICK_UP: MapPin, IN_STORE: Store, SHIPMENT: Truck };
...
function useSaleStatusFlow(sale, enabled) {
  ...
  const currentIndex = forward.findIndex((s) => s.statusId === sale?.status?.statusId);
...
export default function OrderCard({
  ...
  const customerName =
    (isPreSale ? getCustomerName(item) : `${item?.customer?.firstName || ""} ${item?.customer?.lastName || ""}`.trim()) || "Guest";
  ...
  const paymentStatus = isPreSale ? getPreSalePaymentStatus(item) : item?.paymentStatus;
  const subtotal = isPreSale ? item?.info?.saleData?.finalSubTotal : item?.finalSubTotal;
  const lifecycle = isPreSale ? getPreSaleLifecycle(item) : item?.status?.statusId;
  ...
  const nextLabel = saleFlow.nextStatus?.statusId === "COMPLETED" ? "Complete" : "Next";
  const nextClass = saleFlow.nextStatus?.statusId === "COMPLETED" ? LIFECYCLE_ACTION_CLASS.PACKAGED_AND_READY : LIFECYCLE_ACTION_CLASS.PROCESSING;
    ...
    <>
      <Card ref={setNodeRef} style={style} className="w-full gap-3 p-4">
        <div className="flex items-center gap-2">
          ...
          <Badge className={paymentStatus === "PAID_IN_FULL" ? "bg-emerald-600 text-white" : "bg-amber-400 text-black"}>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-medium">{subtotal != null ? `$${Number(subtotal).toFixed(2)}` : "—"}</span>
        </div>

        <div className="flex gap-2">
          <Button size="sm" variant="outline" className="flex-1" onClick={() => setDetailsOpen(true)}>
            Details <ChevronDown className="size-3.5" />
          </Button>

          {isPreSale && lifecycle === "NEW" && (
            <>
              <Button size="sm" variant="outline" className="flex-1 border-destructive text-destructive hover:bg-destructive/10" onClick={() => onCancel(item)}>
                Cancel
              </Button>
              <Button size="sm" className={`flex-1 ${actionClass}`} onClick={() => onConfirm(item)}>
                Confirm
              </Button>
            </>
          )}
          ...
          {isPreSale && lifecycle === "CONFIRMED" && (
            <>
              ...
              <Button size="sm" className={`flex-1 ${actionClass}`} onClick={() => onAssignPackages(item)}>
                Assign Pkgs
              </Button>
            </>
          )}
          ...
          {!isPreSale && (
            <>
              <Button
                ...
                disabled={!saleFlow?.cancelStatus || saleFlow?.updating}
                onClick={() => saleFlow.moveTo(saleFlow.cancelStatus.statusId, onRefresh)}
              >
                Cancel
              </Button>
              <Button
                ...
                disabled={!saleFlow?.nextStatus || saleFlow?.updating}
                onClick={() => saleFlow.moveTo(saleFlow.nextStatus.statusId, onRefresh)}
              >
                {nextLabel}
              </Button>
            </>
      ...
      <OrderDetailDialog open={detailsOpen} onClose={() => setDetailsOpen(false)} type={type} item={item} />
    </>
  );
}
