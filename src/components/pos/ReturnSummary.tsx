...
/**
 * Read-only return summary: purchased line items (with discount/tax breakdown),
 * miscellaneous charges, and the sale/return totals. Pure display — mirrors the
 * old antd Collapse layout with native disclosure elements.
 */
export default function ReturnSummary() {
  ...
  return (
    <div>
      ...
      <div className="flex justify-between pt-5">
        <div className="font-semibold">Return SubTotal</div>
        <div className="font-semibold">
          $ {getOrderSummary?.data?.finalPayable ?? "0.00"}
        </div>
      </div>

      <div className="mt-2 flex justify-between">
        <div className="font-semibold text-muted-foreground">Return Total</div>
        <div className="font-semibold">
          $ {getOrderSummary?.data?.finalPayable ?? "0.00"}
        </div>
      </div>
