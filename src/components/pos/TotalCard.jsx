...
import MiscellaneousCharge from "@/components/pos/MiscellaneousCharge";
 ...
 *    loyalty display), applied-coupon row, MiscellaneousCharge, NewAvailableCoupons,
 ...
 *  - Customer loyalty settings fetch/display card (no loyalty service yet);
 *    loyalty *removal* still routes through ProductPromoTaxes -> deleteLoyaltyPoints.
 */
export default function TotalCard({ refreshOrders, onDraftSaved, setResetOrderHistory }) {
  ...
  const currentAction = useSelector((state) => state?.orderAction?.orderAction);
  ...
  const [storeCreditsReturned, setStoreCreditsReturned] = useState(0);
