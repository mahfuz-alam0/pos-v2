...
// Camera-based package barcode / QR scanner — same live-video scan-loop
// pattern as signin/QrScanDialog.tsx, generalized to every barcode
// symbology (see lib/barcodeScan.ts) instead of QR only.
    ...
    (async () => {
      try {
        ...
        const loop = async () => {
          ...
          const text = await decodeBarcodeFromVideoFrame(
            videoRef.current,
          ).catch(() => null);
          if (cancelled || !scanningRef.current) return;
          if (text) {
