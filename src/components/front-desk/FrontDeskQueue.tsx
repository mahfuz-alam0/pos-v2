...
// Dark-themed reskin of dashboard/CustomerQueue for the Front Desk screen,
// plus the client-side "Search queue…" filter the old app had (the widget
// version doesn't need it — the queue there is short and inline).
    ...
    <div className="rounded-2xl border border-white/10 bg-[#0d1224] p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Users className="size-4.5 text-violet-300" />
          <h2 className="text-base font-semibold text-white">Customer Queue</h2>
      ...
      <div className="relative mb-2.5">
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-[#141a2e] px-3 py-2 text-sm text-white outline-none placeholder:text-white/40 focus:border-violet-400/50"
        />
        {quickFocused && quickResults.length > 0 && (
          <div className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-white/10 bg-[#141a2e] shadow-lg">
            {quickResults.map((c) => (
              <button
                key={c.id}
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full rounded-lg border border-white/10 bg-[#141a2e] py-2 pr-3 pl-8 text-sm text-white outline-none placeholder:text-white/40 focus:border-violet-400/50"
        />
      </div>

      <div className="flex flex-wrap gap-3">
        {loading ? (
          <div className="w-full py-8 text-center text-white/40">Loading…</div>
        ) : filtered.length === 0 ? (
          <div className="w-full py-8 text-center text-white/40">No Data Found</div>
        ) : (
