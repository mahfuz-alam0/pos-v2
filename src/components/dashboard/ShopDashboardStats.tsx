...
function StatBlock({ label, value, isCurrency = true, borderRight = true, borderBottom = false }) {
  return (
    <div
      className="flex min-w-0 flex-col items-center justify-center px-2 py-[22px]"
      style={{
    ...
    >
      <span
        className="leading-[1.1] whitespace-nowrap text-[color:#2A9D8F]"
        style={{ fontSize: "clamp(1rem, 1.4vw, 1.75rem)", fontWeight: 300, letterSpacing: "-0.02em" }}
      >
        {isCurrency ? formatCurrency(value) : value ?? 0}
      </span>
      <span className="mt-[5px] max-w-full overflow-hidden text-center text-[10px] font-medium text-ellipsis whitespace-nowrap text-muted-foreground">
        {label}
      </span>
    </div>
  );
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-component-bg">
      <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-border px-4 py-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={goToPrev}
            className="flex items-center gap-1 rounded-full border border-primary/30 bg-component-bg px-3 py-1 text-sm font-medium text-[color:#2A9D8F] transition-all"
          >
            <ChevronLeft className="size-3" /> Previous
          </button>
          <span className="rounded-full bg-surface-alt px-3 py-1 text-sm font-semibold text-[color:#2A9D8F]">
            {dateShort}
          </span>
          <button
            onClick={goToNext}
