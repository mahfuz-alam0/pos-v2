...
export default function PrinterSelectionModal({ open, onOpenChange, onSelect, defaultJobType = JOB_TYPES.PACKAGE_LABEL }) {
  ...
  // Socket for hardware print-job acknowledgements
  useEffect(() => {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="z-60 sm:max-w-200">
          <DialogHeader>
            <DialogTitle>Select Printer Device</DialogTitle>
          </DialogHeader>

          <Tabs value={activeJobType} onValueChange={setActiveJobType}>
            ...
            <TabsContent value={activeJobType} className="max-h-[50vh] overflow-y-auto pt-2">
              {loading ? (
